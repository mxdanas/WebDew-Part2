...
function renderCart(){
    ...
    cartItems.forEach(item => {
        ...
        const qtyInput = document.getElementById(`qty-${item.product.id}`) as HTMLInputElement;
    ...
    if(totalCost) totalCost.textContent = `$${CartService.getTotal().toFixed(2)}`;
    if (shippingCost) shippingCost.textContent = `$${CartService.getShipping().toFixed(2)}`;
    if (finalTotal) finalTotal.textContent = `$${CartService.getFinalTotal().toFixed(2)}`;
